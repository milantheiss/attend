	...
	<div class="mb-40 m-4 font-ubuntu">
		...
		<nav class="relative container mx-auto mt-6 md:mt-12 mb-8 md:mb-12 md:max-w-medium-width px-2" v-if="auth.authenticated">
			<div class="flex justify-between justify-content-center items-center">
				...
				<router-link @click="showMenu = false" to="/profile">
					<div
						class="absolute -top-2 right-0 z-10 w-6 h-6 bg-delete-gradient-1 rounded-full flex justify-center items-center text-center shadow-lg text-white text-sm font-bold"
						v-if="dataStore.notifications?.length > 0">
						{{ dataStore.notifications.length }}
					</div>
			...
			<transition
				...
				<div class="grid mt-2" v-show="showMenu">
					<router-link
						...
						to="/attendancelist"
						class="text-left font-medium text-xl md:text-2xl mt-2 ml-2"
					...
					<router-link
						...
						class="text-left font-medium text-xl md:text-2xl mt-2 ml-2"
						v-if="auth.user?.lengthAccessibleGroups > 0"
						>Abrechnung erstellen</router-link
					>
					<router-link
						@click="showMenu = !showMenu"
						to="/editgroup"
					...
					<router-link
						...
						class="text-left font-medium text-xl md:text-2xl mt-2 ml-2"
						v-if="auth.user?.lengthAccessibleGroups > 0"
						>Liste exportieren</router-link
					>
					<router-link @click="showMenu = !showMenu" to="/logout" class="text-left font-medium text-xl md:text-2xl mt-2 ml-2">Logout</router-link>
					<p class="text-center font-light text-sm md:text-base mt-2 mx-auto">Erstellt von Milan Theiß - Version 0.1.4</p>
				</div>
			</transition>
		</nav>
		...
		<div v-if="dataStore.showPatchNotesDialog">
			<ModalDialog :show="showPatchNotes">
				...
				<template #footer
					><button
						@click="dataStore.readPatchNotes()"
